-- In this set of exercises, we will work with a billing database for a company that provides web hosting services to its customers. 
-- The database will contain information about its customers and the services each customer uses. 
-- Each customer can have any number of services, and every service can have any number of customers. 
...
-- The customer id in this table should have the property that deleting the corresponding customer record from the customers 
-- table will automatically delete all rows from the join table that have that customer_id. Do not apply this same property to the service id.

-- Each combination of customer and service in the table should be unique. In other words, a customer shouldn't be listed as 
-- using a particular service more than once.
