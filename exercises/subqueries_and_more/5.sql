...
-- For this exercise, you must use a subquery to generate a result table (or transient table), 
...
SELECT max(count) FROM (SELECT count(id) FROM bids GROUP BY bidder_id) AS "max";